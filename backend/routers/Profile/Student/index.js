...
router.get('/', isAuthenticated, isAuthorized(['student']), async (req, res) => {
    ...
    .then(async profile => {
        ...
        .catch(err => {})
        if(profile.projects) {
            profile.projects = await Promise.all(profile.projects.map(async (p) => {
        ...
        if(profile.innovations) {
            profile.innovations = await Promise.all(profile.innovations.map(async (i) => {
        ...
        if(profile.certificates) {
            profile.certificates = await Promise.all(profile.certificates.map(async (c) => {
                if(c.file) {
...
router.post('/profile', isAuthenticated, isAuthorized(['student']), async (req, res) => {
...
router.post('/academic', isAuthenticated, isAuthorized(['student']), async (req, res) => {
    const reqData = req.body
    const updateUserData = async () => {
        const studentData = await User.findOne({ _id: req.userId })
    ...
    try {
        if(reqData.grade || reqData.school) {
            ...
            if(studentProfile) {
                if(reqData.grade)
                    studentProfile.grade = reqData.grade
                if(reqData.school) {
                    if(school.location) {
                        studentProfile.schoolDetails = school
                    }
                    else {
                        studentProfile.schoolDetails = {...studentProfile.school, name: school.name}
                    }
                }
                await studentProfile.save()
                .then(studentProfile => {
                ...
                if(reqData.school) {
                    if(school.location) {
                        ...
                        studentProfile.school = {...studentProfile.school, name: school.name}
                    }
                }
                await studentProfile.save()
                .then(studentProfile => {
