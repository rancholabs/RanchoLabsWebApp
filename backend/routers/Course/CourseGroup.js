...
router.post('/', isAuthenticated, isAuthorized(['admin']), async (req, res) => {
    const error = {message: 'Error in creating course group', error: 'Bad Request'}
...
router.put('/:id', isAuthenticated, isAuthorized(['admin']), async (req, res) => {
    ...
    const error = {message: 'Error in updating course group', error: 'Bad Request'}
    ...
    CourseGroup.updateOne({_id: id}, updateData, (err, raw) => {
        if(err) {
            ...
            res.status(204).send({message: (n > 0) ? `Course group updated successfully` : 'No Changes detected'})
        }
    })
})

router.get('/', async(req, res) => {
    const error = {message: 'Error in retrieving course groups', error: 'Bad Request'}
    //CourseGroup.find()
...
router.get('/:id', async(req, res) => {
    const id = req.params.id
    const error = {message: 'Error in retrieving course group', error: 'Bad Request'}
    CourseGroup.aggregate([
        {
