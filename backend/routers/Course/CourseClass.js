...
router.post('/', async (req, res) => {
    ...
    classes = classes.map((clas, idx) => {
        ...
        if(clas.materials) {
            ...
            if(clas.materials.references) {
                clas.materials.references = clas.materials.references.map((ref, idx) => {
                    if(ref.file) {
    ...
    const error = {message: 'Error in adding course classes', error: 'Bad Request'}
...
router.put('/:id', async (req, res) => {
    ...
    if(updateData.materials) {
        if(updateData.materials.lesson) {
            updateData.materials.lesson = mongoose.Types.ObjectId(updateData.materials.lesson)
        }
        if(updateData.materials.video) {
            updateData.materials.video = mongoose.Types.ObjectId(updateData.materials.video)
        }
        if(updateData.materials.references) {
            updateData.materials.references = updateData.materials.references.map((ref, idx) => {
                if(ref.file) {
                    ref.file = mongoose.Types.ObjectId(ref.file)
                }
                return ref
...
router.delete('/:id', async (req, res) => {
    const id = req.params.id
    const error = {message: 'Error in deleting course class', error: 'Bad Request'}
    Class.deleteOne({_id: id}, (err, raw) => {
        if(err) {
            ...
            const {nModified: n} = raw
            res.status(204).send({message: (n > 0) ? `Course class deleted Succesfully` : 'No Changes detected'})
        }
    })
})
