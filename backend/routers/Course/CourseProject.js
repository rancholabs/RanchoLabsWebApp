...
router.post('/', async (req, res) => {
    ...
    const error = {message: 'Error in adding course projects', error: 'Bad Request'}
...
router.delete('/:id', async (req, res) => {
    const id = req.params.id
    const error = {message: 'Error in deleting course project', error: 'Bad Request'}
    Project.deleteOne({_id: id}, (err, raw) => {
        if(err) {
            ...
            const {nModified: n} = raw
            res.status(204).send({message: (n > 0) ? `Course project deleted Succesfully` : 'No Changes detected'})
        }
    })
})
