...
router.post('/:cid', async (req, res) => {
    ...
    const error = {message: 'Error in adding course batches', error: 'Bad Request'}
    ...
    .then(classes => {
        ...
        .then(projects => {
            ...
            batches = batches.map((batch, idx) => ({...batch, projects: uProjects}))
            Batch.insertMany(batches)
            .then(batch => {
                res.status(201).send({message: 'Course batches added Succesfully'})
            })
            .catch(err => {
...
router.put('/class/:bid/:cid', async (req, res) => {
    ...
    const error = {message: 'Error in updating course class', error: 'Bad Request'}
    ...
    if(Object.keys(reqData).length > 0) {
        ...
        .then(raw => {
            ...
            if((n > 0) && hasStartTime) {
                ...
                .then(courses => {
                    if(courses[0]) {
                        ...
                        sendMail(emails, {type: 'CLASSTIME_UPDATED', args: {courseName:courses[0].courseName, startDate: reqData.startTime}})
                    }
                })
                .catch(err => {
                    console.log(err)
            ...
            res.status(204).send({message: (n > 0) ? `Course class updated Succesfully` : 'No Changes detected'})
        })
        .catch(err => {
            console.log(err)
...
router.put('/project/:bid/:pid', async (req, res) => {
    ...
    const reqData = req.body ? req.body : {}
    const error = {message: 'Error in updating course project', error: 'Bad Request'}
    if(Object.keys(reqData).length > 0) {
        const updateData = {}
        if(reqData.isActive) {
        ...
        Batch.updateOne({_id: bid, 'projects.projectId': pid}, {$set: updateData})
        .then(raw => {
            const {nModified: n} = raw
            res.status(204).send({message: (n > 0) ? `Course project updated Succesfully` : 'No Changes detected'})
        })
        .catch(err => {
            console.log(err)
            res.send(500).send(error)
...
router.delete('/:bid', async (req, res) => {
    ...
    const error = {message: 'Error in deleting course batch', error: 'Bad Request'}
    Batch.deleteOne({_id: bid}, (err, raw) => {
        if(err) {
            ...
            const {nModified: n} = raw
            res.status(204).send({message: (n > 0) ? `Course batch deleted Succesfully` : 'No Changes detected'})
        }
    })
})
