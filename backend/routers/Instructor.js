...
router.post('/', isAuthenticated, isAuthorized(['instructor']), async (req, res) => {
...
router.put('/update', isAuthenticated, isAuthorized(['instructor']), async(req ,res) => {
    try{
        console.log('here')
...
router.get('/', isAuthenticated, isAuthorized(['instructor']), async(req ,res) => {
    try{
...
router.post('/schedule', isAuthenticated, isAuthorized(['instructor']), async(req,res) => {
    try{
        ...
        let getMergedDataClasses = (arr1, arr2) => {
                ...
                arr1.forEach(item => map.set(item.userId.toString(), {...map.get(item.userId.toString()), ...item}))
                return Array.from(map.values())
            }
        classes = classes.map((clas) => {
            if(clas.classDetails && clas.classDetails.attendance && clas.classDetails.attendance.length && clas.students && clas.students.every((s)=>{if(s.userId) return true}))
            {
                clas.students = clas.students.map((s) => {
                s.present = false 
                return s})
                clas.students = getMergedDataClasses(clas.classDetails.attendance, clas.students)
