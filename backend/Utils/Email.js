    ...
    switch(type) {
        case 'ACCOUNT_REGISTERED': 
            return (args) => {
                return {
                    ...
                    mail: `Dear student,
                    <div style="text-indent:5em;padding-top:1em;">Your account has been registered successfully with us.</div><br>
                    Thanks,<br>
                    Rancho Labs`
                }
            }
        ...
        case 'COURSE_REGISTERED':
            return (args) => {
                ...
                const startsOn = ordinal_suffix_of(date.getDate()) + ' ' + month[date.getMonth()] + ', ' + date.getFullYear() + ' (' + day[date.getDay()] + ')'
                return {
        ...
        case 'CLASSTIME_UPDATED':
            return (args) => {
                const {courseName, startDate} = args
                const date = new Date(startDate)
                const startsOn = ordinal_suffix_of(date.getDate()) + ' ' + month[date.getMonth()] + ', ' + date.getFullYear() + ' ' + formatAMPM(date) + ' (' + day[date.getDay()] + ')' 
                return {
        ...
        case 'ABSENT':
            const {courseName, startDate} = args
                const date = new Date(startDate)
                const conductedOn = ordinal_suffix_of(date.getDate()) + ' ' + month[date.getMonth()] + ', ' + date.getFullYear() + ' ' + formatAMPM(date) + ' (' + day[date.getDay()] + ')' 
            return (args) => {
                return {
