...
import { UpdatePreviewConclusion, UpdatePreviewSaveHandler } from '../../Actions/InnovationPreview'
import './css/InnovationComments.css'

const InnovationComments = () => {
    const {conclusion} = useSelector((state) => state.innovationPreview)
    const [comments, setComments] = useState(conclusion ? conclusion : [{id: 0, heading: '', subHeading: '', image: '', description: ''}])
    const updateCommentHandler = (id, attr, value) => {
        const objIndex = comments.findIndex((obj => obj.id === id))
        if(objIndex >= 0) {
            setComments([
                ...comments.slice(0, objIndex),
                {...comments[objIndex], [attr]: value},
                ...comments.slice(objIndex+1)
            ])
        }
    }
    ...
    const readImage = (image, id) => {
        readURL(image, `innovation-comment-image-${id}`)
        return ''
    }

    const addComment = () => {
        const maxId = Math.max.apply(Math, comments.map(function(o) { return o.id; }))
        setComments([...comments, {id: maxId === -Infinity ? 0 : maxId+1, heading: '', image: '', description: ''}])
    }

    const delCommentImage = (id) => {
        updateCommentHandler(id, 'image', '')
    }
    ...
    useEffect(() => {
        const previewSaveHandler = () => {
            dispatch(UpdatePreviewConclusion(comments))
        }
    ...
    return (
        <div className="innovation-comments">
                                            ...
                                            <div>
                                                {c.image ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="info">Add Image if any</div>
                                                        <input type="file" accept="image/*" className="image-file" onChange={(e) => {
                                                            let files = e.target.files
                                                            updateCommentHandler(c.id, 'image', files[0])
                                                        }} />
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                        <textarea className="desc" type="text" placeholder="Content" value={c.description} onChange={(e) => updateCommentHandler(c.id, 'description', e.target.value)} />
                                    </div>
                                </div>
                            )
                        })
            ...
            <div className="add-comment" onClick={() => addComment()}>
...
export default InnovationComments
