...
import { UpdatePreviewHeader, UpdatePreviewSaveHandler } from '../../Actions/ProjectPreview'
...
const ProjectHeading = () => {
    ...
    const {header} = useSelector((state) => state.projectPreview)
    ...
    const readImage = image => {
        readURL(image, "project-heading-image")
        return ''
    }

    const readVideo = video => {
        readURL(video, "project-heading-video")
        return ''
    }
    ...
    useEffect(() => {
        if(header) {
            ...
            if(header.category) {
                if(categories.includes(header.category))
                    setCategory(header.category)
                else
                    setCategory('Others')
                setCategoryOthers(header.category)
            }
            if(header.image) {
                setImage(header.image)
            }
            if(header.video) {
                setVideo(header.video)
            }
        }
    }, [header])
            ...
            <div className="category">
                <div className="label">
                    <div className="box" onClick={() => toggleDropdown()}>
                        <div className="text">
                            {category ? category : "ADD PROJECT CATEGORY"}
                ...
                <div>
                    {image ? (
                        <div className="uploaded">
                            ...
                            <img id="project-heading-image" src={image.filePath ? image.filePath : typeof image === "object" ? readImage(image) : ''} />
                        </div>
                    ) : (
                        <div className="image">
                            ...
                            <div className="detail">
                                ...
                                <div className="info">Add cover Image of the Project</div>
                            </div>
                        </div>
                    )}
                    {video ? (
                        <div className="uploaded">
                            ...
                            <video controls={true}>
                                <source id="project-heading-video" src={video.filePath ? video.filePath : typeof video === "object" ? readVideo(video) : ''} />
                            </video>
                        </div>
                    ) : (
                        <div className="video">
                            ...
                            <div className="detail">
                                ...
                                <div className="info">Add video of the Project</div>
                            </div>
                        </div>
                    )}
                </div>
