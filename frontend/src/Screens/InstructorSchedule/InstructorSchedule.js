...
import { instructorSchedule, instructorInfo, instructorUpdate, instructorUpdateBatchClass, updateClassLink } from '../../Actions/Instructor'
...
const Event = () => {
    return (
        <>
            <div className="event">
                <div className="image"><img className="img-fluid" src={eventimage} /></div>
                ...
                <div className="row mx-0 share-all border-bottom align-items-center">
                    Share all
                        ...
                        <input type="checkbox" id="share-all"
                            onChange={() => { shareAll(!all) }}
                            checked={all} disabled={disable} />
                ...
                <div className="all-links">
                    <div>
                        <div className="row mx-0 align-items-center">
                            Class link
                                                            <label class="switch">
                                <input type="checkbox" id="share-link" onChange={() => { shareLink(!link) }} checked={link} disabled={saveLink} />
                                <span class="slider"></span>
                            </label>
                        </div>
                        <input type="text" placeholder="class link" disabled={saveLink} value={classLink} onChange={(e) => setClassLink(e.target.value)} />
                        <button className="save-link" disabled={saveLink} onClick={saveClassLink}>Save</button>
                    </div>
                    <div className="row mx-0 align-items-center">
                        Share slides
                                                            <label class="switch">
                            <input type="checkbox" id="share-slides" onChange={() => { shareSlides(!slide) }} checked={slide} disabled={disable} />
                            <span class="slider"></span>
                        </label>
                    </div>
                    <div className="row mx-0 align-items-center">
                        Share assignment
                                                        <label class="switch">
                            <input type="checkbox" id="share-assignment" onChange={() => { shareAssignment(!assignment) }} checked={assignment} disabled={disable} />
                            <span class="slider"></span>
                        </label>
                    </div>
                    <div className="row mx-0 align-items-center">
                        Share quiz
                                                        <label class="switch">
                            <input type="checkbox" id="share-quiz" onChange={() => { shareQuiz(!quiz) }} checked={quiz} disabled={disable} />
                            <span class="slider"></span>
                        </label>
                    </div>
                </div>
...
const Results = (props) => {
    ...
    return (
        <>
            <div className="view"><button disabled={disable}>View Quiz Results</button></div>
            <div className="view"><button disabled={disable}>View Submitted Assignments</button></div>
        </>
    )
}
    ...
    return (
        <>
            <div className="instructor-note">
                ...
                <div className="send-note-button"><button disabled={disable} onClick={sendNote}>Send Note</button></div>
            </div>
        </>
    )
}
...
const ClassListCard = (props) => {
    ...
    var timing = (props.startTime && props.endTime) ? formatAMPM(new Date(props.startTime)) + ' - ' + formatAMPM(new Date(props.endTime)) : ''
        ...
        <>
            <div className={className}>
                <div className="row mx-0">
                    ...
                    <div>
                        <div>
                            <div className="row mx-0" style={{ alignItems: "center", padding: "calc(30*var(--vp)) 0 calc(24*var(--vp))" }}>
                                <div>
                                ...
                                <div className="details-button">
                                    <button className="detail" onClick={() => { setSchedule(!ischedule) }}>
...
const Schedule = () => {
    ...
    const [batch, setBatch] = useState(["A", "B", "C"])
    const [date, setDate] = useState(new Date())
    const { userInfo } = useSelector((state) => state.userLogin)
    const { instructorSchedule: schedule } = useSelector((state) => state.instructorSchedule)
    var week = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
    var today = `${week[new Date().getDay()]}, ${new Date().getDate()} ${new Date().toLocaleString('default', { month: 'short' })} ${new Date().getFullYear()}`
    ...
    const classList = schedule && date ? schedule.classes.filter((clas) => {
        if (new Date(clas.startTime).getDate() === date.getDate() && new Date(clas.startTime).getMonth() === date.getMonth() && new Date(clas.startTime).getFullYear() === date.getFullYear())
            return clas
    }) : null
        ...
        <>
            {
                <>
                    <div className="row mx-0 schedule-upper">
                        ...
                        <div className="row mx-0" style={{ justifyContent: "space-between", width: "100%", paddingTop: "calc(29*var(--vp))" }}>
                            <div className="calendar">
                                <input type="date" onChange={(e) => setDate(new Date(e.target.value))} />
                            </div>
                            <select className="batches" onChange={(e) => {
                                ...
                                {batches.map((b) => {
                                    return (
                                        <>
                                            {
                                                b === 'All' ?
                                                    <option value={b} selected>All batches</option> :
