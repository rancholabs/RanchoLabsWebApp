  ...
  return (
    <div className="add-course-dialog">
      ...
      <input
        ...
        placeholder="Enter Category (Eg: Robotics)"
      />
      <textarea
        className="description"
        style={{ textAlign: description.length ? "initial" : "center" }}
        value={description}
        onChange={(e) => setDescriptionHandler(e.target.value)}
        placeholder="Description (65 characters)"
      />
      <div className="actions">
        <button className="cancel" onClick={() => setIsAddCourse(false)}>
          CANCEL
        </button>
        <button
          className={`add${
            !heading || !category || !description ? " disabled" : ""
          }`}
          disabled={!heading || !category || !description}
...
const EditCourse = ({
  ...
  return (
    <div className="edit-course-dialog">
      ...
      />
      <div className="actions">
        <button className="cancel" onClick={() => setIsEditCourse(false)}>
          CANCEL
        </button>
        <button className="delete" onClick={deleteHandler}>
          DELETE
        </button>
        <button className="save" onClick={editCourseClickHandler}>
          SAVE
        </button>
      </div>
    </div>
...
const StudentProfileCourses = () => {
  const { courses: coursesArr, isEditView } = useSelector(
  ...
  useEffect(() => {
    const totalPagesLastIndexTemp = parseInt(
      (courses.length -
        (isEditView && courses.length < rowsPerPage * maxPages ? 0 : 1)) /
        rowsPerPage
    );
      ...
      {isAddCourse ? (
        ...
        <div className="courses-container">
          <div className="pcourses">
            {isEditView &&
              activePage === 0 &&
              courses.length < rowsPerPage * maxPages && (
                <div
            ...
            {courses
              .slice(
                activePage === 0 && courses.length < rowsPerPage * maxPages
                  ? activePage * rowsPerPage
                  : activePage * rowsPerPage - (isEditView ? 1 : 0),
                (activePage + 1) * rowsPerPage -
                  (isEditView && courses.length < rowsPerPage * maxPages
                    ? 1
                    : 0)
              )
              .map((c) => {
                  ...
                  <div key={c.id}>
                    {isEditView && (
                      <StudentProfileEditIcon
                        onClickHandler={() =>
                          setIsEditCourse({ value: true, id: c.id })
                        }
                      />
                    )}
                    ...
                    {isEditView && window.screen.width > 600 && (
                      <button
                        onClick={() => updateCourseIsEnabled(c.id)}
