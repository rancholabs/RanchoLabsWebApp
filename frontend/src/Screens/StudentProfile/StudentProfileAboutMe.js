...
const StudentProfileAboutMe = () => {
  ...
  const [state, setState] = useState({
    ...
    school: schoolDetails
      ? `${schoolDetails.name} - ${schoolDetails.location}`
      : "",
  });
          ...
          <div className="desc">
            {!isEditing ? (
              state.school ? (
                ...
                "School you study in (eg:Delhi Public School)"
              )
            ) : (
              <input
                type="text"
          ...
          <div className="btns">
            <div>
              ...
              <button onClick={saveHandler} className="save-btn">
                SAVE {window.screen.width <= 800 ? "" : "CHANGES"}
              </button>
            </div>
          </div>
        )}
