...
const ItemEdit = ({ idx, value, changeHandler }) => {
  return (
    <input
      onChange={(e) => changeHandler(e.target, idx)}
      type="text"
      value={value ? value : ""}
...
const Items = ({ isEditing, items, setItems }) => {
  const changeHandler = (elem, idx) => {
    ...
    setItems([...items.slice(0, idx), elem.value, ...items.slice(idx + 1)]);
  };
  return (
    <div className="items">
      {isEditing ? (
...
const StudentProfileExtracurricular = () => {
  const { extraCurriculars, isEditView } = useSelector(
  ...
  return (
    <div
      id="student-profile-extracurricular"
      className="student-profile-extracurricular"
    >
      <img src={extracurricularImg} className="icon" />
      <div className={isEditing ? "editing-mode" : ""}>
        {!isEditing && isEditView && (
          <StudentProfileEditIcon onClickHandler={openEditingMode} />
        )}
        <Items
          isEditing={isEditing}
          items={!isEditing ? items : itemsTemp}
        ...
        {isEditing && (
          <div className="btns">
            <div>
              ...
              <button onClick={saveHandler} className="save-btn">
                SAVE CHANGES
              </button>
            </div>
          </div>
