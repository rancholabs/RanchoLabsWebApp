...
const AddSkill = ({skills, setSkills, setIsAddSkill}) => {
    ...
    const addSkillClickHandler = () => {
        if(skill && level) {
            const maxId = Math.max.apply(Math, skills.map(function(s) { return s.id; }))
            const skillsNew = [
                ...skills,
                {
    ...
    return (
        <div className="add-skill">
            <input type="text" value={skill} onChange={(e) => setSkill(e.target.value)} placeholder="Name of the skill" />
            <div className="label">Select the level of understanding</div>
            <button className={`value${level === "beginner" ? " selected" : ""}`} onClick={() => setLevel('beginner')}><div>BEGINNER</div></button>
            <button className={`value${level === "intermediate" ? " selected" : ""}`} onClick={() => setLevel('intermediate')}><div>INTERMEDIATE</div></button>
            <button className={`value${level === "expert" ? " selected" : ""}`} onClick={() => setLevel('expert')}><div>EXPERT</div></button>
            <div className="actions">
                <button className="cancel" onClick={() => setIsAddSkill(false)}>CANCEL</button>
                <button className={`add${(!skill || !level) ? " disabled" : ""}`} disabled={!skill || !level} onClick={addSkillClickHandler}>ADD</button>
            </div>
        </div>
    )
}
...
const StudentProfileSkills = () => {
    const {skills: skillsArr, isEditView} = useSelector(state => state.studentProfile)
    ...
    useEffect(() => {
        const totalPagesLastIndexTemp = parseInt((skills.length > 0 ? skills.length-1 : 0)/rowsPerPage)
                            ...
                            {skills.slice(rowsPerPage * activePage, rowsPerPage * (activePage+1)).map((s, idx) => {
                                return (
                                    <div key={s.id}>
                                        ...
                                        <div className="level">
                                            <div className={s.level === "beginner" ? "level-1" : s.level === "intermediate" ? "level-2" : s.level === "expert" ? "level-3" : "level-0"}>{s.level ? s.level.toUpperCase() : "" }</div>
                                        </div>
                                        {isEditView && <svg onClick={() => delSkillHandler(s.id)} aria-hidden="true" focusable="false" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className="svg-inline--fa fa-times fa-w-10 fa-3x skill-remove-icon"><path fill="currentColor" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" className=""></path></svg>}
                                    </div>
                                )
                            })}
                        </div>
