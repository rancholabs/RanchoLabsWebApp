...
const StudentProfileSideNav = () => {
  ...
  useEffect(() => {
    const hideNameRefOnMatch = () => {
      if (
        (nameContainerRef.current.scrollHeight > 0 &&
          window.scrollY > nameContainerRef.current.scrollHeight) ||
        (navRef.current.scrollHeight > 0 &&
          window.scrollY > navRef.current.scrollHeight)
      ) {
          ...
          {isEditNameAim ? (
            <>
              <input
                ...
                value={
                  typeof editNameState.name === "object"
                    ? `${editNameState.name.first} ${editNameState.name.last}`
                    : editNameState.name
                    ? editNameState.name
                    : ""
                }
                onChange={(e) => editNameStateHandler("name", e.target.value)}
              />
              <input
                type="text"
                placeholder="I want to become a Coder"
                value={editNameState.aim}
                onChange={(e) => editNameStateHandler("aim", e.target.value)}
              />
              <div className="actions">
                <button className="cancel" onClick={cancelNameEditHandler}>
                  CANCEL
                </button>
                <button className="save" onClick={saveNameEditHandler}>
                  SAVE
                </button>
              </div>
            </>
            ...
            <>
              {isEditView && (
                <StudentProfileEditIcon onClickHandler={openEditNameMode} />
              )}
              <div className="name">
                {nameState.name
                  ? `${nameState.name.first} ${nameState.name.last}`
                  : "YOUR NAME"}
              </div>
              <div className="aim">
                {nameState.aim ? nameState.aim : "I want to become a Coder"}
              </div>
            </>
          )}
        </div>
