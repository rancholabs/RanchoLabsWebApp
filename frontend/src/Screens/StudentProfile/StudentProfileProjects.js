...
const StudentProfileProjects = () => {
    const {projects: projectsArr, isEditView} = useSelector(state => state.studentProfile)
    const [projects, setProjects] = useState(projectsArr ? projectsArr : [])
    const [projectsTemp, setProjectsTemp] = useState(projectsArr ? projectsArr : [])
    ...
    useEffect(() => {
        if(projectsArr) {
            if(!isEditView) {
                ...
                setProjects(projectsArr.filter(p => {
                    if(p.isEnabled) {
                        return p
                    }
                }))
    ...
    useEffect(() => {
        const totalPagesLastIndexTemp = parseInt((projects.length-(isEditView && (projects.length < rowsPerPage * maxPages) ? 0 : 1))/rowsPerPage)
        ...
        <div id="student-profile-projects" className="student-profile-projects">
            ...
            <div className="projects-container">
                <div className="projects">
                    ...
                    {projects.slice(activePage === 0 && (projects.length < rowsPerPage * maxPages) ? activePage * rowsPerPage : activePage * rowsPerPage - (isEditView ? 1 : 0), (activePage+1) * rowsPerPage - (isEditView && (projects.length < rowsPerPage * maxPages) ? 1 : 0) ).map(p => {
                        return (
                            <div key={p._id} onClick={() => {window.open(`/project/${p._id}`, '_blank')}} className={!p.isUploaded ? "draft" : ""}>
                                {isEditView && (
                                    <>
                                        <svg onClick={(e) => {
                                            delProjectHandler(p._id)
                                            e.stopPropagation()
                                        }} aria-hidden="true" focusable="false" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className="svg-inline--fa fa-times fa-w-10 fa-3x project-remove-icon"><path fill="currentColor" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" className=""></path></svg>
                                        <StudentProfileEditIcon onClickHandler={() => history.push(`/project?projectId=${p._id}`)} />
                                    </>
                                )}
                                {p.isUploaded ? <img className="image" src={p.header.image && p.header.image.filePath ? p.header.image.filePath : ''} /> : <div className="image">DRAFT</div>}
