...
const StudentProfileInnovations = () => {
  const { innovations: innovationsArr, isEditView } = useSelector(
    (state) => state.studentProfile
  );
  const [innovations, setInnovations] = useState(
    innovationsArr ? innovationsArr : []
  );
  const [innovationsTemp, setInnovationsTemp] = useState(
    innovationsArr ? innovationsArr : []
  );
  ...
  const updateInnovationIsEnabled = (id, attr, value) => {
    ...
    if (objIndex >= 0) {
      const innovationsNew = [
        ...
        {
          ...innovations[objIndex],
          isEnabled: !innovations[objIndex].isEnabled,
        },
  ...
  const delInnovationHandler = (id) => {
    if (
      ...
      window.confirm("Are you sure want to delete this Innovation ?")
    ) {
  ...
  useEffect(() => {
    const totalPagesLastIndexTemp = parseInt(
      (innovations.length -
        (isEditView && innovations.length < rowsPerPage * maxPages ? 0 : 1)) /
        rowsPerPage
    );
  ...
  return (
    <div
      id="student-profile-innovations"
      className="student-profile-innovations"
    >
      ...
      <div className="innovations-container">
        <div className="innovations">
          {isEditView &&
            activePage === 0 &&
            innovations.length < rowsPerPage * maxPages && (
              <div
                className="add-innovation"
                onClick={() => {
          ...
          {innovations
            .slice(
              activePage === 0 && innovations.length < rowsPerPage * maxPages
                ? activePage * rowsPerPage
                : activePage * rowsPerPage - (isEditView ? 1 : 0),
              (activePage + 1) * rowsPerPage -
                (isEditView && innovations.length < rowsPerPage * maxPages
                  ? 1
                  : 0)
            )
            .map((i) => {
              return (
                <div
                  key={i._id}
                  onClick={() => {
                    window.open(`/innovation/${i._id}`, "_blank");
                  }}
                  className={!i.isUploaded ? "draft" : ""}
                >
                  {isEditView && (
                    <>
                      <svg
                        ...
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 320 512"
                        className="svg-inline--fa fa-times fa-w-10 fa-3x innovation-remove-icon"
                      >
                        <path
                          fill="currentColor"
                      ...
                      <StudentProfileEditIcon
                        onClickHandler={() =>
                          history.push(`/innovation?innovationId=${i._id}`)
                        }
                      />
                    </>
                  )}
                  {i.isUploaded ? (
                    <img
                      className="image"
                      src={
                        i.header.image && i.header.image.filePath
                          ? i.header.image.filePath
                          : ""
                      }
                    />
                  ) : (
