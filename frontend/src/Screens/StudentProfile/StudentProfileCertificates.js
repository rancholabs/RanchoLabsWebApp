...
const StudentProfileCertificates = () => {
  const { certificates: certificatesArr, isEditView } = useSelector(
  ...
  const [certificatesTemp, setCertificatesTemp] = useState(
    certificatesArr ? certificatesArr : []
  );
  ...
  const updateCertificateIsEnabled = (id) => {
    ...
    if (objIndex >= 0) {
      const certificatesNew = [
        ...
        {
          ...certificates[objIndex],
          isEnabled: !certificates[objIndex].isEnabled,
        },
  ...
  useEffect(() => {
    const totalPagesLastIndexTemp = parseInt(
      (certificates.length -
        (isEditView && certificates.length < rowsPerPage * maxPages ? 0 : 1)) /
        rowsPerPage
    );
  ...
  return (
    <div
      id="student-profile-certificates"
    ...
    >
      ...
      <div className="certificates-container">
        <div className="certificates">
          {isEditView &&
            activePage === 0 &&
            certificates.length < rowsPerPage * maxPages && (
          ...
          {certificates
            .slice(
              activePage === 0 && certificates.length < rowsPerPage * maxPages
                ? activePage * rowsPerPage
                : activePage * rowsPerPage - (isEditView ? 1 : 0),
              (activePage + 1) * rowsPerPage -
                (isEditView && certificates.length < rowsPerPage * maxPages
                  ? 1
                  : 0)
            )
            .map((c) => {
              return (
                <div key={c.id}>
                  {isEditView && (
                    <svg
                      onClick={() => delCertificateHandler(c.id)}
                      aria-hidden="true"
                      focusable="false"
                      data-prefix="fal"
                      data-icon="times"
                      ...
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 320 512"
                      className="svg-inline--fa fa-times fa-w-10 fa-3x certificate-remove-icon"
                    >
                      <path
                        fill="currentColor"
