...
const StudentProfile = () => {
    const {isEditView, isShareOpen} = useSelector(state => state.studentProfile)
    ...
    useEffect(() => {
        if(!profileId) {
            if(userInfo)
                ...
                history.replace('/login?redirect=/profile/student')
        }
    ...
    return (
        <div className="student-profile">
            ...
            {isShareOpen && <StudentProfileShareBtns type={isShareOpen.type} id={isShareOpen.id} />}
        </div>
    )
}
