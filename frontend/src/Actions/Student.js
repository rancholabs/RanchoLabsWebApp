...
const updateStudent = (updates) => async (dispatch, getState) => {
  try {
    ...
    // CHECK FOR WORKSHOP BATCH
    if (updates.school) {
      const _body = {
        batchId: updates.batch,
    ...
    const { data } = await axios.post("/api/student", updates, config);
