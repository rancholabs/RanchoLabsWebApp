...
export const loginFacebook = (email, token) => async (dispatch) => {
  try {
    ...
    await axios
      .post("/api/login/facebook", { email, token }, config)
      .then((res) => {
        localStorage.setItem("userInfo", JSON.stringify(res.data));
        dispatch({
          type: USER_LOGIN_SUCCESS,
...
export const loginGoogle = (token) => async (dispatch) => {
  try {
    ...
    await axios
      .post("/api/login/google", { token }, config)
      .then((res) => {
        localStorage.setItem("userInfo", JSON.stringify(res.data));
        dispatch({
          type: USER_LOGIN_SUCCESS,
...
export const register = (name, email, password = "") => async (dispatch) => {
  try {
    ...
    await axios
      .post("/api/register", { name, email, password }, config)
      .then((res) => {
        dispatch({
...
export const registerGoogle = (token) => async (dispatch) => {
  try {
    ...
    await axios.post("/api/register/google", { token }, config).then((res) => {
      dispatch({
        type: USER_REGISTER_SUCCESS,
        payload: res.data,
      });
...
export const registerFacebook = (email, token) => async (dispatch) => {
  try {
    ...
    await axios
      .post("/api/register/facebook", { email, token }, config)
      .then((res) => {
        dispatch({
          type: USER_REGISTER_SUCCESS,
          payload: res.data,
...
export const workshop_register = (location, email, grade) => async (
  dispatch
) => {
  try {
    dispatch({
    ...
    axios
      .post("/api/workshop/free/register", { location, email, grade }, config)
      .then((res) => {
        localStorage.setItem("userInfo", JSON.stringify(res.data));
        dispatch({
...
export const logout = (displayAlert = true) => (dispatch) => {
  ...
  if (displayAlert) alert("Logged out successfully");
  dispatch({ type: USER_LOGOUT });
};

export const setUserPassword = (password, reset = false) => async (
  dispatch,
  getState
) => {
  try {
    ...
    const {
      userRegister: { userInfo: regUserInfo },
    } = getState();
...
export const UserForgotPassword = (email) => async (dispatch) => {
  try {
    ...
    await axios
      .post("/api/forgotPassword", { email }, config)
      .then((res) => {
        dispatch({
...
export const sendMessage = (senderName, senderMail, message) => async (
  dispatch,
  getState
) => {
  try {
...
export const callbackRequest = (senderName, senderContact) => async (
  dispatch
) => {
  try {
    dispatch({
