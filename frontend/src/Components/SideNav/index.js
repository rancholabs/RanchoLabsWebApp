...
const SubLink = ({display, link, navCloseHandler}) => {
    return (
        <div>
            {link ? (
                <NavLink to={link} onClick={() => navCloseHandler()}>{display}</NavLink> 
            ): (
                <div style={{color: "#FFFFFF", cursor: "pointer"}} onClick={() => alert('The page is under construction')}>{display}</div>
            )}
        </div>
    )
}
...
const SideNav = ({navCloseHandler}) => {
    ...
    useEffect(() => {
        const onresize = () => {
            if(window.screen.width <= 600 || (isIPadMiniMobileView && window.screen.width <=768)) {
                ...
                sideNavClose.current.getElementsByClassName('side-nav-close')[0].classList.add('mobile')
            }
            else {
                ...
                sideNavClose.current.getElementsByClassName('side-nav-close')[0].classList.remove('mobile')
            }
        }
    ...
    useEffect(() => {
        const listenClick = (e) => {
            if(sideNav.current && e.target !== sideNav.current && !sideNav.current.contains(e.target)) {
                navCloseHandler()
            }
        }
    ...
    return (
        <div className="side-nav" ref={sideNav}>
            <div className="head-line">
                <div className="side-nav-logo" ref={sideNavLogo} onClick={() => goToPage('/')}>
                    <img className="icon" alt="app-icon" src={Logo} />
                ...
                <div className="sub-links">
                    {
                        subLinks.map(({display, link}, si) => {
                            return (<SubLink key={si} display={display} link={link} navCloseHandler={navCloseHandler} />)
                        })
                    }
                </div>
                <div className="email-icon">
                    <FontAwesomeIcon className="icon" color="#3CFAFF" icon={faEnvelope} />
                    <span><a href="mailto:labs.rancho@gmail.com" className="email">labs.rancho@gmail.com</a></span>
                </div>
                {!userInfo ? (
